...
class BM25Retriever:
    ...
    def __init__(self):
        ...
        # StemmingAnalyzer 面向英文分词（按空白/词形切分），对没有空格的中文文本几乎无法正确切词，
        # 会导致中文关键词检索始终查不到结果。改用字符 n-gram 分词器以支持中文子串匹配。
        cjk_analyzer = analysis.NgramAnalyzer(minsize=2, maxsize=3)
        self.schema = Schema(
            node_id=ID(stored=True, unique=True),
            content=TEXT(stored=True, analyzer=cjk_analyzer),
            keywords=TEXT(stored=True, analyzer=cjk_analyzer)
        )
        self._ensure_index()
    ...
    def rebuild_index(self, documents: List[Dict]):
        """重建索引（用于知识库更新时）"""
        # 重新创建一个空索引以清空旧内容，避免重复重建时产生重复/过期文档
...
class RetrievalPipeline:
    ...
    async def sync_bm25_from_kb(self, trace_id: str = "N/A", force: bool = False):
        """从知识库（ChromaDB）全量同步 BM25 索引。
        force=False 时仅在索引为空时才重建，避免每次启动都重复重建。
        """
        if not force and self.bm25_retriever.doc_count() > 0:
            logger.bind(trace_id=trace_id).info(
                f"BM25 索引已有 {self.bm25_retriever.doc_count()} 条文档，跳过同步"
            )
            return
        nodes = await external_kb.get_all_nodes(trace_id=trace_id)
        if nodes:
            self.bm25_retriever.rebuild_index(nodes)
        else:
            logger.bind(trace_id=trace_id).warning("知识库为空，BM25 索引未同步")
