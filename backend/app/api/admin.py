...
from app.models import (
    ...
    CozeWorkflowRegistry, PracticeNodeQuestion, PracticeQuestionDraft
)
...
async def delete_kb_node(node_id: str):
    ...
    # 同步 BM25 索引，避免关键词检索路仍能检索到已删除的节点
...
# ==========================================
# V6.0 Coze 工作流驱动的"练习/通关"模块 · 管理端
# ==========================================

# --- CozeWorkflowRegistry Schemas ---
class CozeWorkflowRegistryCreate(BaseModel):
    business_line: str
    workflow_id: str
...
class CozeWorkflowRegistryUpdate(BaseModel):
...
async def list_coze_workflows(db: AsyncSession = Depends(get_db)):
    """获取所有业务线的 Coze 工作流注册记录"""
    stmt = select(CozeWorkflowRegistry).order_by(CozeWorkflowRegistry.business_line.asc())
    result = await db.execute(stmt)
    return result.scalars().all()


@router.post("/coze-workflows")
async def create_coze_workflow(payload: CozeWorkflowRegistryCreate, db: AsyncSession = Depends(get_db)):
    """注册一个业务线对应的 Coze 工作流"""
    existing = await db.execute(
        select(CozeWorkflowRegistry).where(CozeWorkflowRegistry.business_line == payload.business_line)
    )
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=400, detail=f"业务线「{payload.business_line}」已注册过工作流，请改用编辑")
    registry = CozeWorkflowRegistry(id=str(uuid.uuid4()), **payload.model_dump())
    db.add(registry)
    await db.commit()
    return registry
...
async def update_coze_workflow(registry_id: str, payload: CozeWorkflowRegistryUpdate, db: AsyncSession = Depends(get_db)):
    registry = await db.get(CozeWorkflowRegistry, registry_id)
    if not registry:
        raise HTTPException(status_code=404, detail="工作流注册记录不存在")
...
# --- PracticeNodeQuestion Schemas ---
class PracticeNodeQuestionCreate(BaseModel):
    business_line: str
...
class PracticeNodeQuestionUpdate(BaseModel):
    node_key: Optional[str] = None
...
async def create_practice_node(payload: PracticeNodeQuestionCreate, db: AsyncSession = Depends(get_db)):
    node = PracticeNodeQuestion(id=str(uuid.uuid4()), **payload.model_dump())
    db.add(node)
    await db.commit()
    return node
...
async def update_practice_node(node_id: str, payload: PracticeNodeQuestionUpdate, db: AsyncSession = Depends(get_db)):
    node = await db.get(PracticeNodeQuestion, node_id)
    if not node:
        raise HTTPException(status_code=404, detail="题目节点不存在")
    updates = payload.model_dump(exclude_unset=True)
    content_fields_changed = any(f in updates for f in ("question_text", "reference_answer", "key_points"))
    for field, value in updates.items():
        setattr(node, field, value)
    # 题干/参考答案/得分点被改动，且管理员没有同时显式设置 sync_status，
    # 说明这份题目又和 Coze 工作流里已经存在的内容不一致了，自动打回"未同步"，
    # 避免出现"内容明明改了，同步状态却还显示已同步"的误导
    if content_fields_changed and "sync_status" not in updates:
        node.sync_status = "unsynced"
    await db.commit()
    return node
...
async def mark_practice_node_synced(node_id: str, db: AsyncSession = Depends(get_db)):
    """纯人工标记：管理员已经把这道题的文案手动同步进 Coze 工作流编辑器，系统无法程序化校验"""
    node = await db.get(PracticeNodeQuestion, node_id)
    if not node:
        raise HTTPException(status_code=404, detail="题目节点不存在")
    node.sync_status = "synced"
    node.last_synced_at = datetime.now()
    await db.commit()
    return node


# --- PracticeQuestionDraft Schemas（与 KnowledgeDraft 审核流程同构）---
class PracticeQuestionDraftGenerate(BaseModel):
    business_line: str
    source_node_id: str
    difficulty: str = "medium"


class PracticeQuestionDraftUpdate(BaseModel):
...
class PracticeQuestionDraftApprove(BaseModel):
    node_key: Optional[str] = None
    order_index: int = 0


@router.post("/practice-question-drafts/generate")
async def generate_practice_question_draft(payload: PracticeQuestionDraftGenerate, db: AsyncSession = Depends(get_db)):
    """管理员手动选一个知识库节点，调用 QuizAgent.generate_question 起草一道练习题草稿"""
    try:
        generated = await quiz_agent.generate_question(
            payload.source_node_id, difficulty=payload.difficulty,
            trace_id=f"practice-draft-{uuid.uuid4().hex[:8]}"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"AI 起草题目失败: {e}")
...
async def update_practice_question_draft(draft_id: str, payload: PracticeQuestionDraftUpdate, db: AsyncSession = Depends(get_db)):
...
async def approve_practice_question_draft(draft_id: str, payload: PracticeQuestionDraftApprove, db: AsyncSession = Depends(get_db)):
    """审核通过：写入 PracticeNodeQuestion（题目镜像表），sync_status 从 unsynced 开始——
    还需要管理员手动把文案粘贴进 Coze 工作流编辑器，这一步系统做不到"""
    draft = await db.get(PracticeQuestionDraft, draft_id)
    if not draft:
        raise HTTPException(status_code=404, detail="草稿不存在")
    if draft.status != "pending":
