...
class DocAgent:
    ...
    async def answer_query(self, query: str, trace_id: str = "N/A", user_profile: dict = None) -> str:
        ...
        # 3. 构造 Prompt
        prompt = f"""
你是一个专业的银行客服培训导师。请优先基于以下提供的参考资料回答考生的提问。
如果参考资料和考生提问明显无关、或不足以回答问题，就基于你自己掌握的通用知识来回答，
但必须在回答开头加一行【提示：以下内容不是本行知识库收录内容，仅供参考，请以官方文档或权威渠道为准】，
不要假装这是知识库里的内容，也不要为了套用参考资料而牵强附会。
...
"""
        messages = [
            {"role": "system", "content": "你是一个专业的银行客服培训导师，优先使用给定参考资料作答，资料不相关或不够用时可以切换到自己的通用知识回答，但要明确提示考生这部分不是知识库内容。"},
